...
type GoodsQuery struct {
	...
	// eager-loading edges.
	withCategory *GoodsCategoryQuery
...
func (gq *GoodsQuery) QueryCategory() *GoodsCategoryQuery {
	query := &GoodsCategoryQuery{config: gq.config}
...
func (gq *GoodsQuery) Clone() *GoodsQuery {
	...
	return &GoodsQuery{
		...
		withCategory: gq.withCategory.Clone(),
...
func (gq *GoodsQuery) WithCategory(opts ...func(*GoodsCategoryQuery)) *GoodsQuery {
	query := &GoodsCategoryQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withCategory = query
	return gq
}
...
func (gq *GoodsQuery) sqlAll(ctx context.Context) ([]*Goods, error) {
	...
	if query := gq.withCategory; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Goods)
		for i := range nodes {
		...
		query.Where(goodscategory.IDIn(ids...))
